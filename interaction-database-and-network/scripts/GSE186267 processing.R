...
sce$cell.type <- ifelse(sce$integrated_snn_res.0.2 %in% 5, "epi","immune")
...
DotPlot(epi, features = epi.markers, group.by = "integrated_snn_res.1", assay = "RNA") + RotatedAxis()
at1 <- 1
at2 <- 0
...
immu <- FindClusters(immu, resolution = 1)
DotPlot(immu, features = immu.markers, group.by = "integrated_snn_res.1", assay = "RNA") + RotatedAxis()
immu$cell.type[immu$integrated_snn_res.1 %in% 5] <- "B cell"
immu$cell.type[immu$integrated_snn_res.1 %in% 8] <- "Plasma cell"
immu$cell.type[immu$integrated_snn_res.1 %in% c(0,7)] <- "T cell"
immu$cell.type[immu$integrated_snn_res.1 %in% 3] <- "NK cell"
immu$cell.type[immu$integrated_snn_res.1 %in% 2] <- "Neutrophil"
immu$cell.type[immu$integrated_snn_res.1 %in% 1] <- "Mast cell"
immu$cell.type[immu$integrated_snn_res.1 %in% 6] <- "Dendritic cell"
immu$cell.type[immu$integrated_snn_res.1 %in% c(4,9)] <- "Monocyte"
...
allsample <- list.files(pattern =  ".rds$")[-6]
