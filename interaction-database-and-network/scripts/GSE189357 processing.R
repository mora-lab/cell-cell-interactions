...
epi.markers <- c("AGER","PDPN","CLIC5","CLDN18","FSTL3", # Alveolar Epithelial Type 1 Cell
                 ...
                 "TM4SF1","CRABP2","UBE2C" # tumor cell
)
...
epi$cell.type <- ifelse(epi$integrated_snn_res.0.2 %in% at1, "AT1",
                        ifelse(epi$integrated_snn_res.0.2 %in% at2, "AT2",
                               ifelse(epi$integrated_snn_res.0.2 %in% tm, "Tumor cell","Airway epithelial")))
...
immu$cell.type[immu$integrated_snn_res.0.5 %in% c(2,17)] <- "B cell"
immu$cell.type[immu$integrated_snn_res.0.5 %in% c(10,16,18)] <- "Plasma cell"
immu$cell.type[immu$integrated_snn_res.0.5 %in% c(0,1,5,12)] <- "T cell"
immu$cell.type[immu$integrated_snn_res.0.5 %in% c(3,13)] <- "NK cell"
immu$cell.type[immu$integrated_snn_res.0.5 %in% c(7,11)] <- "Neutrophil"
immu$cell.type[immu$integrated_snn_res.0.5 %in% 4] <- "Mast cell"
immu$cell.type[immu$integrated_snn_res.0.5 %in% c(8,9,14)] <- "Macrophage"
immu$cell.type[immu$integrated_snn_res.0.5 %in% 15] <- "Dendritic cell"
immu$cell.type[immu$integrated_snn_res.0.5 %in% 6] <- "Monocyte"
...
allsample <- list.files(pattern =  ".rds$")[-1]
