run_CellPhoneDB_SSP = function(posi,allparis,gold){
  posi <- posi[,1:4]
  ...
  TP <- as.numeric(nrow(posi[posi$V5 %in% gold$V5,]) + nrow(posi[posi$V6 %in% gold$V5,]))
  FP <- as.numeric(nrow(posi) - TP)
  FN <- as.numeric(nrow(gold) - TP)
  TN <- as.numeric(nrow(allpairs) - (TP + FP + FN))
  
  # calculate accuracy, sensitivity and specificity
  acc <- as.numeric((TP+TN)/nrow(allpairs))
  pre <- TP/(TP+FP)
  sst <- TP/(TP+FN)
  spc <- TN/(FP+TN)
  f1 <- 2*TP/(2*TP+FP+FN)
